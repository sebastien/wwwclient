...
def listAgents(data=DATA):
	agents = data.keys()
	agents.sort()
	return agents

def pickAgent(data=DATA):
	return random.choice(listAgents())

def listVersions(agent, data=DATA):
	if agent in data:
		res =  data[agent].keys()
		res.sort(lambda a,b:cmp(int(a.split(".")[0]), int(b.split(".")[0])))
		return res
	else: 
		return []
