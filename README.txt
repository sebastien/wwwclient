...
----------------
    ...
    You can get the forms in the current session by invoking:

    >   session.last().forms()

    Which will return you a `dict` with form names as key and @scrape.Form as
    values. Each form can be easily manipulated using the following methods:
